...
import {
  listMcqs, createMcq, updateMcq, deleteMcqs, changeMcqStatus, moveMcqs, bulkImportMcqs, getMcqStats,
  type McqRow, type McqStatus, type McqListResult,
...
function McqManagerPage() {
  ...
  const createMut = useMutation({
    mutationFn: (payload: {
      ...
      correctIndex: number; explanation: string; status: McqStatus;
    }) => createFn({ data: payload }),
    onSuccess: () => { toast.success("MCQ created"); invalidateAll(); },
  ...
  const updateMut = useMutation({
    mutationFn: (payload: {
      ...
      explanation: string; status: McqStatus;
    }) => updateFn({ data: payload }),
    onSuccess: () => { toast.success("MCQ updated"); invalidateAll(); },
    onError: (e) => toast.error((e as Error).message),
  });
      ...
      <div className="relative grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4">
        <div className="min-w-0">
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
            ...
            <span className="rounded-md bg-secondary/70 px-2 py-1 text-foreground">MCQ Manager</span>
          </nav>
          <div className="mt-3 flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h1 className="truncate text-2xl font-bold tracking-tight text-foreground sm:text-3xl">MCQ Manager</h1>
...
function KpiGrid({ kpis }: { kpis: Record<string, number> }) {
  const items = [
    { key: "total", label: "Total MCQs", value: kpis.total, icon: Database, tone: "primary" as KpiTone },
...
function McqTableRow({
  ...
  row: McqRow; serial: number; selected: boolean; onToggle: () => void; striped: boolean;
...
function McqCard({ row, selected, onToggle, onView, onEdit, onMove, onDelete }: {
  row: McqRow; selected: boolean; onToggle: () => void;
  onView: () => void; onEdit: () => void;
  onMove: () => void; onDelete: () => void;
}) {
  return (
...
function ViewModal({ row, onClose, onEdit }: { row: McqRow; onClose: () => void; onEdit: () => void }) {
  return (
...
function EditModal({ row, tree, onClose, onSave }: {
  row: McqRow; tree: ApiLevel[]; onClose: () => void;
...
function MoveModal({ rows, tree, onClose, onConfirm }: {
  rows: McqRow[]; tree: ApiLevel[]; onClose: () => void;
...
function DeleteModal({ rows, onClose, onConfirm }: {
  rows: McqRow[]; onClose: () => void; onConfirm: () => Promise<void> | void;
}) {
      ...
      <div className="space-y-4 px-6 py-5">
        <div className="rounded-xl border border-destructive/30 bg-destructive/[0.06] p-3 text-xs text-destructive">
          <div className="flex items-center gap-2 font-semibold">
            ...
            You are about to permanently remove {rows.length.toLocaleString()} MCQ{rows.length > 1 ? "s" : ""}.
          </div>
