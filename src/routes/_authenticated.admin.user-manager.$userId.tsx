...
import {
  ArrowLeft, BadgeCheck, Ban, CalendarClock, CheckCircle2, Crown,
  KeyRound, LogOut, Mail, Pencil, Phone, ShieldCheck, ShieldOff,
  Sparkles, Trash2, X, XCircle,
} from "lucide-react";
import {
  changeUserRole, deleteUsers, forceLogout, getUser, sendPasswordReset,
  setUsersBanned, updateUserProfile,
  type AdminUserDetail, type UserStatus,
...
const initials = (name: string, email: string) => {
  ...
  return s.split(/\s+|@/).filter(Boolean).map((n) => n[0]).slice(0, 2).join("").toUpperCase();
};
...
function UserDetailsPage() {
  ...
  const [confirm, setConfirm] = useState<null | {
    title: string; body: string; tone: "danger" | "warning" | "primary";
    cta: string; action: () => Promise<unknown> | unknown;
  }>(null);
  ...
  const run = async (fn: () => Promise<unknown> | unknown, ok: string) => {
    try { await fn(); setToast(ok); invalidate(); }
    catch (e) { setToast((e as Error).message || "Something went wrong"); }
  };
  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-md rounded-2xl border border-border/60 bg-card/60 p-8 text-center shadow-sm backdrop-blur-xl">
        ...
        <Link to="/admin/user-manager" className="mt-5 inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-br from-primary to-accent px-3.5 py-2 text-sm font-semibold text-primary-foreground shadow">
  ...
  const joinedAgoDays = Math.max(1, Math.floor((Date.now() - new Date(user.createdAt).getTime()) / 86_400_000));

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm">
        <Link to="/admin/user-manager" className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-muted-foreground transition hover:bg-secondary/60 hover:text-foreground">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-1.5">
                <h1 className="truncate text-xl font-bold tracking-tight sm:text-2xl">{user.fullName || "—"}</h1>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                ...
                <Chip><CalendarClock className="h-3 w-3" /> Joined {joinedAgoDays}d ago</Chip>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <ActionBtn
                ...
                onClick={() => setConfirm({
                  ...
                  tone: "primary", cta: "Send link",
                  action: () => run(() => resetFn({ data: { email: user.email, redirectTo: typeof window !== "undefined" ? `${window.location.origin}/reset-password` : undefined } }), "Password reset link sent"),
                })}
              />
              <ActionBtn
                icon={LogOut} label="Force logout" tone="warning"
                onClick={() => setConfirm({
                  title: "Force logout all sessions?",
                  body: "The user will be signed out from every device.",
                  tone: "warning", cta: "Sign out",
                  action: () => run(() => signOutFn({ data: { userId: user.id } }), "All sessions signed out"),
                })}
              />
              <ActionBtn
                icon={Crown} label={user.role === "admin" ? "Demote" : "Promote"}
                onClick={() => setConfirm({
                  title: `Change role to ${user.role === "admin" ? "Student" : "Admin"}?`,
                  body: user.role === "admin" ? "User will lose admin privileges." : "User will gain full admin privileges.",
                  tone: user.role === "admin" ? "warning" : "primary",
                  cta: "Change role",
                  action: () => run(() => roleFn({ data: { userId: user.id, role: user.role === "admin" ? "student" : "admin" } }), "Role updated"),
                })}
              />
              {user.status === "active" ? (
                <ActionBtn
                  icon={Ban} label="Disable" tone="warning"
                  onClick={() => setConfirm({
                    title: `Disable ${user.fullName || user.email}?`,
                    body: "The user will be unable to sign in until reactivated.",
                    tone: "warning", cta: "Disable",
                    action: () => run(() => banFn({ data: { userIds: [user.id], banned: true } }), "User disabled"),
                  })}
                />
              ) : (
                <ActionBtn
                  icon={CheckCircle2} label="Activate" tone="success"
                  onClick={() => run(() => banFn({ data: { userIds: [user.id], banned: false } }), "User activated")}
                />
              )}
              <ActionBtn
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card title="Profile" className="lg:col-span-1">
          ...
          <FieldRow icon={CalendarClock} label="Registration date" value={fmtDateTime(user.createdAt)} />
          ...
          <ToggleRow
            ...
            onToggle={() =>
              setConfirm({
                ...
                action: () =>
                  run(
                    () => roleFn({ data: { userId: user.id, role: user.role === "admin" ? "student" : "admin" } }),
                    "Role updated",
                  ),
              })
            }
      ...
      <EditDialog
        ...
        onSave={async (p) => {
          await run(() => updateFn({ data: { userId: user.id, fullName: p.fullName, phone: p.phone } }), "Profile updated");
          setEditing(false);
        }}
      />
...
function Card({ title, children, className }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <section className={`rounded-2xl border border-border/60 bg-card/60 shadow-sm backdrop-blur-xl ${className ?? ""}`}>
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 transition hover:bg-secondary/40">
      <div className="min-w-0">
        ...
        <div className={`mt-0.5 flex items-center gap-1.5 text-sm font-medium text-foreground ${mono ? "font-mono" : ""}`}>
...
function ToggleRow({ icon: Icon, label, hint, enabled, onToggle, danger, readOnly }: {
  icon: React.ComponentType<{ className?: string }>;
  label: string; hint: string; enabled: boolean; onToggle?: () => void; danger?: boolean; readOnly?: boolean;
}) {
  const onCls = danger ? "bg-rose-500" : "bg-gradient-to-r from-primary to-accent";
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 transition hover:bg-secondary/40">
      <div className="flex min-w-0 items-center gap-3">
        <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-border/60 ${enabled ? (danger ? "bg-rose-500/10 text-rose-500" : "bg-primary/10 text-primary") : "bg-background/60 text-muted-foreground"}`}>
      ...
      >
        <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow ring-1 ring-black/5 transition ${enabled ? "translate-x-6" : "translate-x-1"}`} />
      </button>
    </div>
  );
}
...
function BigAvatar({ user }: { user: AdminUserDetail }) {
  ...
  return (
    <div className="relative">
      {user.photoUrl ? (
        <img src={user.photoUrl} alt="" className="h-16 w-16 rounded-2xl object-cover ring-4 ring-card sm:h-20 sm:w-20" />
      ) : (
        <div
      ...
      <span className={`absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full border-2 border-card ${user.status === "active" ? "bg-emerald-500" : "bg-rose-500"}`} />
    </div>
  );
}
...
function ActionBtn({ icon: Icon, label, onClick, tone }: {
  ...
  const cls =
    tone === "danger" ? "border-rose-500/40 text-rose-600 hover:bg-rose-500/10 dark:text-rose-400"
  ...
  return (
    <button onClick={onClick} className={`inline-flex items-center gap-1.5 rounded-xl border bg-background/60 px-3 py-2 text-xs font-semibold shadow-sm backdrop-blur transition hover:-translate-y-0.5 ${cls}`}>
...
function ModalShell({ open, onClose, children, size = "md" }: {
...
const inputCls = "h-10 w-full rounded-xl border border-border/70 bg-background/70 px-3 text-sm text-foreground outline-none transition focus:border-primary/50 focus:ring-4 focus:ring-primary/15";

function EditDialog({ open, user, onClose, onSave }: {
  open: boolean; user: AdminUserDetail; onClose: () => void;
  onSave: (p: { fullName: string; phone: string }) => void | Promise<void>;
}) {
  const [name, setName] = useState(user.fullName);
  const [phone, setPhone] = useState(user.phone);
  const [saving, setSaving] = useState(false);
  useEffect(() => { if (!open) return; setName(user.fullName); setPhone(user.phone); }, [open, user]);

  return (
    <ModalShell open={open} onClose={onClose} size="md">
      <form onSubmit={async (e) => { e.preventDefault(); setSaving(true); await onSave({ fullName: name, phone }); setSaving(false); }}>
        ...
        <div className="grid grid-cols-1 gap-3 p-6 sm:grid-cols-2">
          ...
          <Field label="Email (read-only)"><input value={user.email} readOnly className={`${inputCls} bg-secondary/40 text-muted-foreground`} /></Field>
          <Field label="Phone"><input value={phone} onChange={(e) => setPhone(e.target.value)} className={inputCls} /></Field>
          <Field label="Role (read-only)"><input value={user.role === "admin" ? "Admin" : "Student"} readOnly className={`${inputCls} bg-secondary/40 text-muted-foreground`} /></Field>
        </div>
...
function ConfirmDialog({ data, onClose }: {
  data: null | { title: string; body: string; tone: "danger" | "warning" | "primary"; cta: string; action: () => Promise<unknown> | unknown };
  onClose: () => void;
}) {
      ...
      {data && (
        <div className="p-6">
          ...
          <div className="mt-5 flex items-center justify-end gap-2">
            ...
            <button
              disabled={busy}
              onClick={async () => { setBusy(true); try { await data.action(); } finally { setBusy(false); onClose(); } }}
